...
An improved, object-oriented driver for managing the LCD temperature display.

FEATURES
  • Auto-detects the working HID interface.
  • Supports temperature display in Celsius (°C) and Fahrenheit (°F).
  • Smart auto-detection of CPU temperature sensors (AMD k10temp, Intel coretemp, etc.).
  • Keeps the panel alive with periodic updates.
  • Subcommand to install udev rules for non-root access.
  • Subcommand to generate and install a systemd service.
"""
...
try:
    ...
    try:
        ...
        print("Error: No HID library found. Install with 'sudo apt install python3-hid' or 'pip install hidapi'")
        sys.exit(1)
...
# Fallback CPU temperature sensor names for Linux
...
class OcypusController:
    ...
    def open(self) -> bool:
        ...
        # Sort devices to try interface 1 first (the LCD display interface)
...
def find_sensor_by_substring(sensors: Dict[str, List[Any]], 
    ...
    """
    Smart sensor finder.
    If substring is 'auto' or not found directly, falls back to common CPU sensors.
    """
    if not sensors:
        return None

    # 1. Direct search if not 'auto'
    ...
    # 2. Try known CPU sensor fallbacks
    ...
    # 3. Fallback to first available sensor with > 0 reading
    for sensor_name, sensor_list in sensors.items():
        if sensor_list:
            for sensor in sensor_list:
                if hasattr(sensor, 'current') and sensor.current > 0:
...
def build_temperature_report(sensor_substring: str = DEFAULT_SENSOR_SUBSTR) -> str:
    ...
    for sensor_name, sensor_list in sensors.items():
        for sensor in sensor_list:
            ...
            highlight = " ← SELECTED" if sensor_name == selected_name else ""
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        Examples:
          ...
          %(prog)s on                      # Start temperature display (Celsius, auto-detect sensor)
          ...
          %(prog)s install-udev            # Install udev rule to run without sudo
    ...
    on_parser.add_argument('-s', '--sensor', default=DEFAULT_SENSOR_SUBSTR,
                          help='Sensor name/substring or "auto" for auto-detection (default: auto)')
    ...
    # Install udev command
    subparsers.add_parser('install-udev', help='Install udev rule for non-root access')
    ...
    service_parser.add_argument('-s', '--sensor', default=DEFAULT_SENSOR_SUBSTR,
                               help='Sensor substring for the service (default: auto)')
